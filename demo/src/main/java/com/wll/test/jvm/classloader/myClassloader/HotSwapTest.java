...
public class HotSwapTest {
    ...
    private static Logger logger = LoggerFactory.getLogger(HotSwapTest.class);
    ...
    private static class ClassFileChangeListenerThread extends Thread {
        ...
        public void run() {
            try {
                File file = new File(HotSwapTest.class.getResource("").getFile() + "Worker.class");
                long lastTime = file.lastModified();
                boolean isFirst = true;
                while (true) {
                    Thread.sleep(2000);
                    File newFile = new File(HotSwapTest.class.getResource("").getFile() + "Worker.class");
    ...
    private static void reloadFile(File newFile, MethodExcuteThread methodExcuteThread) {
        ...
        try {
            Class<?> objClass = hotClassLoader.findClass("Worker");
            //把这个新的CLASS设置到另一个线程中
